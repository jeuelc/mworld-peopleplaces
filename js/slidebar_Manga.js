/* i-C-a */
$('.menubtn').on('tap', function() {
});

$('.homebtnholder').on('tap', function() {
  $.mobile.changePage("sub-modernworld.html");
});

$('.cpbtnholder').on('tap', function() {
  $.mobile.changePage("controlpanel.html");
  mainaudiobg.play(); bgsoundManga.pause(); bgsoundManga.currentTime = 0; 
});

$('.menubtnManga').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.menubtnManga').css('background-position', '0px -90px');
    }, 300);
  var housingpos = $('.menuhousing').position();
  if(housingpos.left == -234) {
    $('.menuhousing').animate({'left':'-334px','opacity':0}, 500, "easeInBack", function() { $('.menuhousing').css({'left':'-660px'}); });
    $('.homebtnholder').animate({'left':'3px','opacity':0}, 500, "easeInBack", function() { $('.homebtnholder').css({'left':'-93px'}); });
    $('.mpbtnholder').animate({'left':'80px','opacity':0}, 500, "easeInBack", function() { $('.mpbtnholder').css({'left':'-93px'}); });
    $('.cpbtnholder').animate({'left':'153px','opacity':0}, 500, "easeInBack", function() { $('.cpbtnholder').css({'left':'-93px'}); });
    $('.infobtnholder').animate({'left':'222px','opacity':0}, 500, "easeInBack", function() { $('.infobtnholder').css({'left':'-93px'}); });
    $('.credsbtnholder').animate({'left':'288px','opacity':0}, 500, "easeInBack", function() { $('.credsbtnholder').css({'left':'-93px'}); });
  } else {
    $('.menuhousing').css({'opacity':1});
    $('.credsbtnholder').css({'opacity':1});
    $('.infobtnholder').css({'opacity':1});
    $('.cpbtnholder').css({'opacity':1});
    $('.mpbtnholder').css({'opacity':1});
    $('.homebtnholder').css({'opacity':1});
    $('.menuhousing').animate({'left':'-234px'}, 1000, "easeInOutBack");
    $('.credsbtnholder').animate({'left':'368px'}, 1000, "easeInOutBack");
    $('.infobtnholder').animate({'left':'302px'}, 1100, "easeInOutBack");
    $('.cpbtnholder').animate({'left':'233px'}, 1200, "easeInOutBack");
    $('.mpbtnholder').animate({'left':'160px'}, 1300, "easeInOutBack");
    $('.homebtnholder').animate({'left':'83px'}, 1400, "easeInOutBack");
  }
});

$('.homebtnManga').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.homebtnManga').css('background-position', '0px -74px');
    }, 300);
});

$('.mpbtnManga').on('tap', function() {
  $(this).css('background-position', '0px -3px');
  setTimeout(function() {
      $('.mpbtnManga').css('background-position', '0px -84px');
    }, 300);
});

$('.cpbtnManga').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.cpbtnManga').css('background-position', '0px -74px');
    }, 300);
});

$('.infobtnManga').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.infobtnManga').css('background-position', '0px -74px');
    }, 300);
});

$('.credsbtnManga').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.credsbtnManga').css('background-position', '0px -74px');
    }, 300);
});

$('.micbtnManga').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.micbtnManga').css('background-position-y', '-81px');
    }, 300);
  var botsposition = $('.bManga').position();
  if (botsposition.left == 1024) {
    $('.bManga').css({'opacity':1});
    $('.plManga').css({'opacity':1});
    $('.fManga').css({'opacity':1});
    $('.bManga').animate({'left':'742px'}, 1000, "easeInOutBack");
    $('.plManga').animate({'left':'808px'}, 1100, "easeInOutBack");
    $('.fManga').animate({'left':'876px'}, 1200, "easeInOutBack");
  } else {
    $('.bManga').animate({'left':'842px','opacity':0}, 500, "easeInBack", function() { $('.bManga').css({'left':'1024px'}); });
    $('.plManga').animate({'left':'908px','opacity':0}, 500, "easeInBack", function() { $('.plManga').css({'left':'1024px'}); });
    $('.fManga').animate({'left':'976px','opacity':0}, 500, "easeInBack", function() { $('.fManga').css({'left':'1024px'}); });
  }
});

$('.rwManga').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.rwManga').css('background-position-y', '-73px');
    }, 300);
});

$('.plpsManga').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.plpsManga').css('background-position-y', '-73px');
    }, 300);
});

$('.fwManga').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.fwManga').css('background-position-y', '-73px');
    }, 300);
});