/* i-C-a */
$('.menubtn').on('tap', function() {
});

$('.homebtnholder').on('tap', function() {
  $.mobile.changePage("sub-naturalworld.html");
});

$('.cpbtnholder').on('tap', function() {
  $.mobile.changePage("controlpanel.html");
  mainaudiobg.play(); bgsoundbluesea.pause(); bgsoundbluesea.currentTime = 0; 
});

$('.menubtndeep').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.menubtndeep').css('background-position', '0px -90px');
    }, 300);
  var housingpos = $('.menuhousing').position();
  if(housingpos.left == -234) {
    $('.menuhousing').animate({'left':'-334px','opacity':0}, 500, "easeInBack", function() { $('.menuhousing').css({'left':'-660px'}); });
    $('.homebtnholder').animate({'left':'3px','opacity':0}, 500, "easeInBack", function() { $('.homebtnholder').css({'left':'-93px'}); });
    $('.mpbtnholder').animate({'left':'80px','opacity':0}, 500, "easeInBack", function() { $('.mpbtnholder').css({'left':'-93px'}); });
    $('.cpbtnholder').animate({'left':'153px','opacity':0}, 500, "easeInBack", function() { $('.cpbtnholder').css({'left':'-93px'}); });
    $('.infobtnholder').animate({'left':'222px','opacity':0}, 500, "easeInBack", function() { $('.infobtnholder').css({'left':'-93px'}); });
    $('.credsbtnholder').animate({'left':'288px','opacity':0}, 500, "easeInBack", function() { $('.credsbtnholder').css({'left':'-93px'}); });
  } else {
    $('.menuhousing').css({'opacity':1});
    $('.credsbtnholder').css({'opacity':1});
    $('.infobtnholder').css({'opacity':1});
    $('.cpbtnholder').css({'opacity':1});
    $('.mpbtnholder').css({'opacity':1});
    $('.homebtnholder').css({'opacity':1});
    $('.menuhousing').animate({'left':'-234px'}, 1000, "easeInOutBack");
    $('.credsbtnholder').animate({'left':'368px'}, 1000, "easeInOutBack");
    $('.infobtnholder').animate({'left':'302px'}, 1100, "easeInOutBack");
    $('.cpbtnholder').animate({'left':'233px'}, 1200, "easeInOutBack");
    $('.mpbtnholder').animate({'left':'160px'}, 1300, "easeInOutBack");
    $('.homebtnholder').animate({'left':'83px'}, 1400, "easeInOutBack");
  }
});

$('.homebtndeep').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.homebtndeep').css('background-position', '0px -74px');
    }, 300);
});

$('.mpbtndeep').on('tap', function() {
  $(this).css('background-position', '0px -3px');
  setTimeout(function() {
      $('.mpbtndeep').css('background-position', '0px -84px');
    }, 300);
});

$('.cpbtndeep').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.cpbtndeep').css('background-position', '0px -74px');
    }, 300);
});

$('.infobtndeep').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.infobtndeep').css('background-position', '0px -74px');
    }, 300);
});

$('.credsbtndeep').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.credsbtndeep').css('background-position', '0px -74px');
    }, 300);
});

$('.micbtndeep').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.micbtndeep').css('background-position-y', '-81px');
    }, 300);
  var botsposition = $('.bdeep').position();
  if (botsposition.left == 1024) {
    $('.bdeep').css({'opacity':1});
    $('.pldeep').css({'opacity':1});
    $('.fdeep').css({'opacity':1});
    $('.bdeep').animate({'left':'742px'}, 1000, "easeInOutBack");
    $('.pldeep').animate({'left':'808px'}, 1100, "easeInOutBack");
    $('.fdeep').animate({'left':'876px'}, 1200, "easeInOutBack");
  } else {
    $('.bdeep').animate({'left':'842px','opacity':0}, 500, "easeInBack", function() { $('.bdeep').css({'left':'1024px'}); });
    $('.pldeep').animate({'left':'908px','opacity':0}, 500, "easeInBack", function() { $('.pldeep').css({'left':'1024px'}); });
    $('.fdeep').animate({'left':'976px','opacity':0}, 500, "easeInBack", function() { $('.fdeep').css({'left':'1024px'}); });
  }
});

$('.rwdeep').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.rwdeep').css('background-position-y', '-73px');
    }, 300);
});

$('.plpsdeep').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.plpsdeep').css('background-position-y', '-73px');
    }, 300);
});

$('.fwdeep').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.fwdeep').css('background-position-y', '-73px');
    }, 300);
});